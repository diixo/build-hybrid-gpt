...
def resolve_warmup_config(train_config: TrainerConfig, world_size: int):
    batch_size = max(1, int(train_config.batch_size))
    grad_accum_steps = max(1, int(train_config.grad_accum_steps))
    ...
    if world_size <= 1:
        return normalized_config, True, effective_batch_size

    if effective_batch_size % world_size != 0:
        return normalized_config, False, effective_batch_size

    per_rank_effective_batch = effective_batch_size // world_size
    per_rank_batch_size = max(1, math.gcd(batch_size, per_rank_effective_batch))
    per_rank_grad_accum_steps = max(1, per_rank_effective_batch // per_rank_batch_size)

    resolved_config = replace(
        train_config,
        batch_size=per_rank_batch_size,
        grad_accum_steps=per_rank_grad_accum_steps,
    )
    return resolved_config, True, effective_batch_size
...
def run_warmup_stage(
    ...
    resolved_config, can_distribute_warmup, effective_batch_size = resolve_warmup_config(
        train_config,
        ddp_world_size,
    )
    use_distributed_warmup = ddp and can_distribute_warmup

    if ddp and not use_distributed_warmup:
        if master_process:
            print(
                f"warmup effective_batch_size={effective_batch_size} is not divisible by world_size={ddp_world_size}; "
                "falling back to rank-0 warmup to preserve single-GPU optimizer semantics"
            )
        if not master_process:
            return model, [], []
    elif use_distributed_warmup and master_process:
        print(
            f"warmup DDP config: global_batch_size={effective_batch_size}, "
            f"per_rank_batch_size={resolved_config.batch_size}, grad_accum_steps={resolved_config.grad_accum_steps}"
        )
    ...
    dataset = WikipediaTextDataset(
        ...
        max_seq_length=MAX_LEN,
        max_rows=max_rows,
        process_rank=ddp_rank if use_distributed_warmup else 0,
        num_processes=ddp_world_size if use_distributed_warmup else 1,
        master_process=master_process,
    )
    if len(dataset) == 0:
        if use_distributed_warmup:
